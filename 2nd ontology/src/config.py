"""
Configuration settings for the Adaptive HMI Ontology generation with improved merging strategy.
"""
...
# Ontology generation configuration
...
Generate the ontology in Turtle format:
"""

# Ontology merging strategy
MERGING_STRATEGY = """
Follow these five key principles for merging ontologies:

1. PRESERVE BOTH COMPETENCY QUESTIONS:
   - Ensure the merged ontology can answer both original competency questions
   - Maintain all essential classes and properties from both ontologies
   - Keep restrictions that are necessary for query answering

2. ELIMINATE DUPLICATES THROUGH PROPERTY CONSOLIDATION:
   - Identify similar properties with different names (e.g., 'respondsTo' vs 'responds')
   - Choose the most descriptive and consistent naming
   - Use inverse property declarations to reduce redundant restrictions

3. MAINTAIN CLEAR NAMING AND RELATIONSHIPS:
   - Use consistent naming patterns across all properties
   - Add clear rdfs:label and rdfs:comment for all entities
   - Maintain logical hierarchy in class and property structures

4. BALANCE EXPRESSIVITY WITH SIMPLICITY:
   - Include only essential restrictions that serve the competency questions
   - Avoid over-complex property chains unless necessary
   - Use domain and range constraints to reduce explicit restrictions

5. USE INVERSE PROPERTIES TO REDUCE REDUNDANCY:
   - Declare inverse relationships explicitly (e.g., owl:inverseOf)
   - Reduce duplicate restrictions by relying on inverse property inference
   - Focus restrictions on the most important direction of the relationship

Apply these principles when merging the ontologies while preserving their ability to answer the original competency questions.
"""
...
All of them need to have an explanation in the rdfs:label and rdfs:comment when needed. 
You also need to add restrictions, and subclasses for both classes and object properties when applicable.
Focus on creating a clear hierarchy and meaningful relationships.
"""